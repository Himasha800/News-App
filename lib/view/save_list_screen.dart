...
class _SaveListScreenState extends State<SaveListScreen> {
  late Future<List<Map<String, dynamic>>> savedNews;
  ...
  void _fetchSavedNews() {
    savedNews = DBHelper().getNewsList(DBHelper().savedNewsTable);
  }

  Future<void> _deleteNews(int id) async {
    try {
      await DBHelper().deleteNews(DBHelper().savedNewsTable, id);
      setState(_fetchSavedNews);
